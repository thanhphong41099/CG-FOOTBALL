...
{
    public class HomePlayersRepo : BaseRepository
    {
        public HomePlayersRepo(SQLiteConnection c) : base(c) { }
        ...
        public void CopyFromTeam(int matchId, int teamId)
        {
            ...
            {
                cmd.CommandText = @"
INSERT INTO HOME_PLAYERS(PLAYER_ID, TEAM_ID, STT, Name, PLAY, DOB, Nationality,
...
FROM PLAYERS WHERE TEAM_ID=@tid;";
        ...
        public void SetPlayByStt(int matchId, int cutoff)
        {
            using (var cmd = Conn.CreateCommand())
            {
                cmd.CommandText = @"UPDATE HOME_PLAYERS
        ...
        public void UpdateStt(int matchId, int playerId, int stt)
        {
            using (var cmd = Conn.CreateCommand())
            {
                cmd.CommandText = @"UPDATE HOME_PLAYERS SET STT=@stt WHERE MATCH_ID=@mid AND PLAYER_ID=@pid;";
        ...
        public List<HomePlayer> List(int matchId)
        {
            var list = new List<HomePlayer>();
            using (var cmd = Conn.CreateCommand())
            {
                cmd.CommandText = @"SELECT PLAYER_ID, TEAM_ID, STT, Name, PLAY, DOB, Nationality,
                                           JerseyNo, JerseyName, Position, Title, Card, Pos1,
                                           TShirt_LineUp1, TShirt_Sub1, TShirt_LineUp2, TShirt_Sub2, MATCH_ID
                                    FROM HOME_PLAYERS WHERE MATCH_ID=@mid
